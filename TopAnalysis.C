      ...
      if(passGRL)
      {
        if(hasGoodVertex)
        {
          ...
          if(goodlep_n==1)
          {
            ...
            if(goodjet_n >= 4)
              {
                //At least two b-tagged jets (only ttbar selection)
                if(goodbjet_n >= 1) //Change in a ZPrime search, at least one b-tagged jet
                  {
                   ...
                   if(Lepton1_MeT_MT > 30000.)
                      {
                       ...
                       // Let's use all the objects
                         //TLorentzVector definitions
                       TLorentzVector j3 = TLorentzVector ();
                                       j3.SetPtEtaPhiE(jet_pt[goodjet_index[jet_three]], jet_eta[goodjet_index[jet_three]], jet_phi[goodjet_index[jet_three]], jet_E[goodjet_index[jet_three]]);
                                       TLorentzVector j4 = TLorentzVector ();
                                       j4.SetPtEtaPhiE(jet_pt[goodjet_index[jet_four]], jet_eta[goodjet_index[jet_four]], jet_phi[goodjet_index[jet_four]], jet_E[goodjet_index[jet_four]]);
                       
                         //Now calculate invariant mass of the sum of those objects and fill histogram
                    ...
                    double names_of_global_variable[]={InvMass1_inGeV, missingEt, vxp_z, (double)pvxp_n, Lepton1_MeT_MT/1000., mass3_max, mass2_max, mazz};
